...
import { durationTypes } from "../data/date";
import { useTranslation } from 'react-i18next';

const DropMod = styled.select <{ theme: string }>`
  background-color: ${(props) =>
    props.theme === "light" ? "#C9D5D8" : "#27798b"};
  border: ${(props) =>
    props.theme === "light" ? "1px solid #27798b" : "none"};
  width: 100%;
  border-radius: 8px;
  ...
  &:hover {
    background-color: ${(props) =>
    props.theme === "light" ? "#FEFEFE" : "#224D57"};
    color: white;
  }
  &:active {
    background-color: ${(props) =>
...
const DropDownContainer = styled.div`
  ...
  flex-grow: 1;
`;
export function DurationDropdownButton({ onSelect }:
  {
      ...
      <Dropdown className="">
        <DropMod theme={theme} defaultValue="" onChange={handleChange}>
          <option value="" disabled>
            {t("Call duration") || t('Select an option')}
          </option>
